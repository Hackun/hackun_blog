...
/* 邮件通知 by Qiqiboy */
 ...
 function SimPaled_send_email($parent_id,$comment){//发送邮件的函数 by Qiqiboy.com
     ...
     if ($spam_confirmed != 'spam' && $to != $admin_email && $to != $author_email) {
         $wp_email = 'hackun.liu@gmail.com'; // e-mail 發出點, no-reply 可改為可用的 e-mail.
         $subject = '您在 [' . get_option("blogname") . '] 的留言有了回應';
         $message = '
             <div style="width:90%;margin:0 auto;background-color:#F2F2F2;" >
             <p>' . trim(get_comment($parent_id)->comment_author) . ', 您好!</p>
             <p>您曾在[ <a href="' . get_option('home') . '">' . get_option('blogname') . '</a>的《' . get_the_title($comment->comment_post_ID) . '》上发表评论:<br /></p>
             <div style="background-color:#ddd;border:1px solid #d8e3e8;color:#111;padding:0 15px;-moz-border-radius:5px;-webkit-border-radius:5px;-khtml-border-radius:5px;">
             <p>' . trim(get_comment($parent_id)->comment_content) .' </p></div>
             <p>' . trim($comment->comment_author) . ' 给你的回复如下:<br /></p>
             <div style="background-color:#ddd;border:1px solid #d8e3e8;color:#111;padding:0 15px;-moz-border-radius:5px;-webkit-border-radius:5px;-khtml-border-radius:5px;">
             <p>' . trim($comment->comment_content) . '</p></div>
